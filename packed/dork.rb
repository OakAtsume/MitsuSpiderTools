...
def generateConsent
  time = Time.new
  "CONSENT=YES+shp.gws-#{time.strftime("%Y%m%d")}-0-RC1.#{SecureRandom.alphanumeric(2).downcase}+FX+740"
end
...
def escapeHex(string)
  string.gsub(/\\x([0-9A-Fa-f]{2})/) { |match| $1.hex.chr }
end
...
def to_unicode(value, encoding = "UTF-8")
  ...
  case value
  when String
    value # If already a string, return as is
  when Array
    value.map { |v| to_unicode(v, encoding) } # Recursively handle arrays
  else
    begin
      value.to_s.encode("UTF-8") # Convert other types to UTF-8 string
