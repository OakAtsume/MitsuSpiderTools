...
class SnowBlood
  def initialize(settings = {})
    ...
    if @settings.key?("agent") && !@settings["agent"].empty?
  ...
  def urihandle(path, base = nil)
    begin
      ...
      path.gsub!(/[^a-zA-Z0-9_\-.:\/~?&=]/) { |char| exclude.include?(char) ? char : URI.encode_uri_component(char) }
  ...
  def extract(body, base)
    ...
    urls.each do |url|
      ...
      begin
        ...
        # Check for inbound
        if @settings["inbound"]
          next unless base.host == uri.host
        end
        links << uri
    ...
    forms.each do |form|
      begin
        ...
        if @settings["inbound"]
          ...
          next unless base.host == formURI.host
        end
    ...
    while pages.any?
      ...
      if req.is_a?(Net::HTTPRedirection) || req.is_a?(Net::HTTPMovedPermanently) || req.is_a?(Net::HTTPFound) || req.is_a?(Net::HTTPSeeOther) || req.is_a?(Net::HTTPTemporaryRedirect) || req.is_a?(Net::HTTPPermanentRedirect)
        ...
        if @settings["inbound"]
          ...
          next unless location.host == page.host
        end
        pages.push(location)
      end
      ...
      log(level: :httplog, message: "#{page} f(#{pages.length}) l(#{visited.length})", code: req.code, msg: "#{req.msg}")
    ...
    params.each do |param|
      ...
      if req.body.include?(payload)
        ...
        if @settings["xss"]
          ...
          if req.body.include?(payload)
            ...
            @vulns.push({ url: uri, type: "XSS", payload: payload, response: "Reflection Of Payload Vector (<'\">)", param: param[0] })
            if !@settings["savevulns"].empty?
              File.open(@settings["savevulns"], "a") { |f| f.write("#{uri} # XSS : #{param[0]} : Reflection Of Payload Vector (<'\">)\n") }
            end
          end
        end
        if @settings["lfi"]
          if req.body.match?(/(?i)[^\n]{0,100}(no such file|failed (to )?open)[^\n]{0,100}/)
            ...
            @vulns.push({ url: uri, type: "LFI", payload: "", response: "Reflection Of Payload Vector (no such file)", param: param[0] })
            # @vulnerable << { url: uri, type: "LFI", payload: "", response: "Reflection Of Payload Vector (no such file)", param: param[0] }
            if !@settings["savevulns"].empty?
              File.open(@settings["savevulns"], "a") { |f| f.write("#{uri} # #{param[0]}  Reflection Of Payload Vector (<'\">)\n") }
            end
          end
        end
      end
      ...
      if @settings["sqli"]
        ...
        payloads.each do |payload|
          ...
          if flags.any? { |flag| req.body.match?(flag) }
            flag = flags.select { |flag| req.body.match?(flag) }
            log(level: :heuristicSQLi, message: "SQLi found in: #{uri} (#{param[0]}) Payload: (#{payload}) Flag: #{flag}")
            @vulns.push({ url: uri, type: "SQLi", payload: payload, response: "Payload: (#{payload}) Flag: #{flag}", param: param[0] })
            if !@settings["savevulns"].empty?
              File.open(@settings["savevulns"], "a") { |f| f.write("#{uri} # SQLi : #{param[0]} : Reflection Of Payload Vector (#{payload})\n") }
            end
